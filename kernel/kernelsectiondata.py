...
class KernelSectionData(KernelSection):
    def __init__(self, game_data: GameData, data_hex: bytearray, id: int, own_offset: int, subsection_nb_text_offset: int,
    ...
    def init_subsection(self, subsection_sized: int, nb_subsection: int):
        for i in range(nb_subsection):
            self.add_subsection(self._data_hex[i * subsection_sized: i * (subsection_sized + 1)])
    ...
    def get_all_offset(self):
        offset_list = []
        print(f"offset_list start: {offset_list}")
        for subsection in self._subsection_list:
            print(f"subsection.get_all_offset(): {subsection.get_all_offset()}")
            offset_list.extend(subsection.get_all_offset())
            print(f"offset_list for: {offset_list}")
